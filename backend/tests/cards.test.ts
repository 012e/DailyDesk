...
describe("Cards API Integration Tests", () => {
  ...
  describe("Card Members", () => {
    ...
    test("should retrieve card with member details via GET", async () => {
      ...
      expect(members).toHaveLength(1);
      expect(members[0]).toHaveProperty("id", boardMemberId);
      expect(members[0]).toHaveProperty("name", "Test User");
      expect(members[0]).toHaveProperty("email", "testuser@example.com");
      expect(members[0]).toHaveProperty("avatar", "https://example.com/avatar.jpg");
      expect(members[0]).toHaveProperty("initials");
    });
